	...
	<div class="container-fluid">
		<div class="col-sm-4">
			<div class="text-center">
				<button class="btn btn-sm btn-info" v-on:click="change(1)">믿고
					보는 맛집 리스트</button>
				<button class="btn btn-sm btn-success" v-on:click="change(2)">지역별
					맛집 리스트</button>
				<button class="btn btn-sm btn-warning" v-on:click="change(3)">메뉴별
					맛집 리스트</button>
			</div>
						...
						<table class="table" v-for="fvo in food_list">
							<tr>
								<td width=30% class="text-center" rowspan="4"><img
									:src="fvo.poster" style="width: 100%" class="img-rounded ddd"
									v-on:click="foodDetail(fvo.fno)"></td>
								<td width=70%>
									<h3 v-on:click="foodDetail(fvo.fno)" class="ddd">
										{{fvo.name }}&nbsp;<span style="color: orange">{{fvo.score
											}}</span>
									</h3>
								</td>
							</tr>
		...
		<div class="col-sm-5" v-show="isShow">
			...
			<table class="table">
				<tr>
					<td colspan="2">
						<h3>
							{{vo2.name }}&nbsp;<span style="color: orange">{{vo2.score
								}}</span>
						</h3>
					</td>
				</tr>
